import axios from 'axios'
import { createMemoryHistory, match } from 'react-router'
import createRoutes from '../../app/routes'
import configureStore from '../../app/store/configureStore'
import { GET_ALL_DATA } from '../../app/types'
import { baseURL } from '../../config/app'
import pageRenderer from './pageRenderer'
import fetchDataForRoute from '../../app/utils/fetchDataForRoute'

import { serverConfig } from '../../config/localConfig'
import { isObjEmpty } from '../../app/helpers/helpers'

// configure baseURL for axios requests (for serverside API calls)
axios.defaults.baseURL = baseURL

/*
 * Export render function to be used in server/config/routes.js
 * We grab the state passed in from the server and the req object from Express/Koa
 * and pass it into the Router.run function.
 */
export default function render(req, res) {
  let preferedLanguages = req.headers["accept-language"]
  preferedLanguages = preferedLanguages ? preferedLanguages.split(';')[0] : ''

  const history = createMemoryHistory()
  const store = configureStore({}, history)
  const routes = createRoutes(store)

  /*
   * From the react-router docs:
   *
   * This function is to be used for server-side rendering. It matches a set of routes to
   * a location, without rendering, and calls a callback(err, redirect, props)
   * when it's done.
   *
   * The function will create a `history` for you, passing additional `options` to create it.
   * These options can include `basename` to control the base name for URLs, as well as the pair
   * of `parseQueryString` and `stringifyQuery` to control query string parsing and serializing.
   * You can also pass in an already instantiated `history` object, which can be constructed
   * however you like.
   *
   * The three arguments to the callback function you pass to `match` are:
   * - err:       A javascript Error object if an error occurred, `undefined` otherwise.
   * - redirect:  A `Location` object if the route is a redirect, `undefined` otherwise
   * - props:     The props you should pass to the routing context if the route matched,
   *              `undefined` otherwise.
   * If all three parameters are `undefined`, this means that there was no route found matching the
   * given location.
   */
  match({ routes, location: req.url }, (err, redirect, props) => {
    if (err) {
      res.status(500).json(err)
    } else if (props) {
      // This method waits for all render component
      // promises to resolve before returning to browser
      var status = 200
      var pathname = props.location.pathname
      var is404 = props.params.splat || (props.routes[1] && props.routes[1].path === "*")
      var isRoot = pathname === '/'

      /* Habilitar si es necesario - Se hace un fetch de los datos de una ruta cuando se ingresa a una ruta directamente
        desde el back
      Promise.all([fetchData]).then(data => {
        if (is404) {
          status = 404
        } else {
          if (data[0]) {
            if (isRoot) {
              store.dispatch({ type: GET_ALL_DATA, data: data })
            }
          }
        }

        const html = pageRenderer(store, props)
        res.status(status).send(html)
      }).catch(err => {
        console.error(err)
        res.status(500).json(err)
      })*/
      const html = pageRenderer(store, props)
      res.status(status).send(html)
    } else {
      res.sendStatus(404)
    }
  })
}
